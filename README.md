...
- **费用验收**：记录启动前、关机前、关机后及关机 5 分钟后的余额，计算实际观察扣费。
- **费用历史**：使用 `/cost` 查看最近一次，使用 `/costs` 查看最近 5 次和有效平均小时成本。
...
- 真实费用以 Layer3 账单为准。
- 开关机完成需要连续两次确认状态文字，并核对电源按钮已经切换为相反动作，避免单一页面信号误报。
...
正式配置使用 `AUTO_SHUTDOWN_SECONDS=3600` 和 `POST_SHUTDOWN_BILLING_CHECK_SECONDS=300`。旧的 `AUTO_STOP_MINUTES` 仍兼容，但只在未设置秒级配置时生效。
