...
如果 Layer3 页面改版导致自动识别失败，机器人会回退到手动输入项目标识和机器名。此时不需要重新输入账号密码，继续按提示输入项目和机器即可完成绑定。项目标识是机器详情页 URL 里的 `/app/projects/<项目标识>/...`，不一定是页面显示的项目名称。
