...
5. 发送 `/bind`，按提示输入 Layer3 登录信息和机器信息
6. 机器人自动检查机器状态
7. 检查成功后，在 Telegram 里使用按钮启动或关闭机器
...
在 Telegram 里发送 `/bind` 后，机器人会依次询问：

- Layer3 登录邮箱
- Layer3 登录密码
- Layer3 项目标识，例如 `default-828`；这是机器详情页 URL 里的 `/app/projects/<项目标识>/...`，不一定是页面显示的项目名称
